...
(function() {
 ...
 === */
  ...
  /* â”€â”€ 9. UNIFIED INTERACTIVE CANDLESTICK CHART ENGINE (LightweightCharts + WebGL) â”€â”€ */
  class InteractiveCandleChart {
        ...
        try {
          ...
          if (window.ResizeObserver) {
            this.resizeObserver = new ResizeObserver(() => {
              if (this.chart && this.container && this.container.clientWidth > 0) {
                this.chart.applyOptions({
                  width: this.container.clientWidth,
                  height: this.container.clientHeight || h
                });
              }
            });
          ...
          console.error('Error mounting LightweightCharts:', e);
        }
      }

      // Fallback Canvas with Drag Pan & Wheel Zoom
      this.mountFallbackCanvas(stock);
    }

    renderData(stock) {
      if (!this.candleSeries) return;
      const ltp = stock?.ltp || 1000;
      const count = 180;
      const candles = [];
      const volumes = [];
      let basePrice = ltp * 0.72;
      const now = new Date();

      for (let i = count; i >= 0; i--) {
        const d = new Date(now.getTime() - (i * 86400000));
        const dateStr = d.toISOString().split('T')[0];
        const change = (Math.random() - 0.48) * (basePrice * 0.022);
        const open = basePrice;
        const close = Math.max(10, open + change);
        const high = Math.max(open, close) + (Math.random() * basePrice * 0.012);
        const low = Math.max(5, Math.min(open, close) - (Math.random() * basePrice * 0.012));
        const vol = Math.floor(15000 + Math.random() * 95000);
      ...
      candles[candles.length - 1].close = ltp;
      candles[candles.length - 1].high = Math.max(candles[candles.length - 1].high, ltp);
    ...
    mountFallbackCanvas(stock) {
      const canvas = document.createElement('canvas');
      canvas.width = this.container.clientWidth || 600;
      canvas.height = this.container.clientHeight || 340;
      ...
      ctx.fillText(`${stock?.symbol || 'STOCK'} Candlestick Engine Active (LTP: \u20B9${(stock?.ltp || 0).toFixed(2)})`, 24, 36);
    }
...
class Application {
 ...
 /* === UI Event Bindings === */
 bindUI() {
    // 0. Modal Candlestick Popout & Timeframe
    document.getElementById('btnModalPopoutChart')?.addEventListener('click', () => {
      const sym = this.currentModalStock?.symbol || 'TRENT';
      window.open(`chart.html?symbol=${encodeURIComponent(sym)}`, '_blank');
    });

    document.querySelectorAll('#modalTimeframeGroup .tv-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        document.querySelectorAll('#modalTimeframeGroup .tv-btn').forEach(b => b.classList.remove('active'));
        btn.classList.add('active');
        if (this.modalCandleChart && this.currentModalStock) {
          this.modalCandleChart.mount(this.currentModalStock);
        }
      });
    });

    // Mobile Navigation & View Toggles
    document.getElementById('btnGoMobile')?.addEventListener('click', () => { window.location.href = 'mobile.html'; });
    document.getElementById('btnTopGoMobile')?.addEventListener('click', () => { window.location.href = 'mobile.html'; });

    // Live Streaming Simulator Toggle
    document.getElementById('btnToggleLive')?.addEventListener('click', () => {
      this.isLiveActive = !this.isLiveActive;
      this.showToast(`Live Market Feed Simulator: ${this.isLiveActive ? 'ACTIVE' : 'PAUSED'}`, this.isLiveActive ? 'success' : 'info');
    });

    // Market Sessions Modal
    const closeSessions = () => document.getElementById('marketSessionsModal')?.classList.remove('active');
    document.getElementById('btnCloseSessionsModal')?.addEventListener('click', closeSessions);
    document.getElementById('btnCloseSessionsModal2')?.addEventListener('click', closeSessions);

    // SmartAPI Management Buttons
    document.getElementById('btnConnectSmartApi')?.addEventListener('click', async () => {
      ...
      const code = document.getElementById('txtSmartApiClientCode')?.value || '';
      const pwd = document.getElementById('txtSmartApiPassword')?.value || '';
      const totp = document.getElementById('txtSmartApiTotp')?.value || '';
      const jwt = document.getElementById('txtSmartApiJwt')?.value || '';

      const res = await AngelOneSmartApiService.authenticate(key, code, pwd, totp, jwt);
      if (res.success) {
    ...
    document.getElementById('btnDisconnectSmartApi')?.addEventListener('click', () => {
      ...
      this.showToast('Angel One SmartAPI Session Disconnected.', 'info');
    });
    ...
    // Multi-Timeframe Green Slider & Checkbox
    document.getElementById('chk_p10')?.addEventListener('change', (e) => {
      this.filters.requireMtfGreen = e.target.checked;
      const wrap = document.getElementById('wrap_mtfGreenSlider');
      const pill = document.getElementById('pill_mtfGreen');
      if (wrap) wrap.style.display = e.target.checked ? 'block' : 'none';
      if (pill) pill.style.display = e.target.checked ? 'inline-flex' : 'none';
      this.runScan();
    });

    document.getElementById('rng_mtfGreen')?.addEventListener('input', (e) => {
      const val = parseInt(e.target.value, 10) || 4;
      this.filters.minMtfGreen = val;
      const valEl = document.getElementById('val_mtfGreen');
      if (valEl) valEl.textContent = `${val}/6 Green`;
      const pill = document.getElementById('pill_mtfGreen');
      if (pill) pill.textContent = `>= ${val}/6 Green`;
      this.runScan();
    });
